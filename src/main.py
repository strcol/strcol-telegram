...
def tick(config, database, vk, telegram):
...
def report_error(config, database, error):
    try:
        connection = database.connect()[config.get('DATABASE_NAME')]
        connection.errors.insert_one({
            'timestamp': int(time.time() * 1000),
            'error': error
        })
    except Exception:
...
def main():
    ...
    while True:
        try:
            tick(config, database, vk, telegram)
        except Exception:
            report_error(config, database, traceback.format_exc())
